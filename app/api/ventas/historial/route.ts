...
export async function GET() {
  ...
  try {
    ...
    const porFecha = new Map<string, { fecha: string; clientes: number; exportado: boolean; rucula: number; lechuga: number }>();
    for (const v of ventas) {
      if (!v.fecha) continue;
      const f = String(v.fecha).split(/[\sT]/)[0];
      const ex = porFecha.get(f) || { fecha: f, clientes: 0, exportado: false, rucula: 0, lechuga: 0 };
      ex.clientes++;
