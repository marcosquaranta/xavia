...
// GraficoCiclos.tsx
// Gráfico SVG de barras: plantas en F1/F2 agrupadas por semana de ciclo.
// Muestra una línea vertical indicando la semana estimada de cosecha.

const COLORES = {
  lechuga_f1: '#86efac',  // verde claro
  lechuga_f2: '#16a34a',  // verde oscuro
  rucula_f1:  '#6ee7b7',  // esmeralda claro
  rucula_f2:  '#047857',  // esmeralda oscuro
};

export interface BarraSemana {
  semana: number;       // semana de ciclo (1, 2, 3...)
  plantas_f1: number;
  plantas_f2: number;
  lotes: string[];      // ids de lotes en esa semana
}
...
export default function GraficoCiclos({
  titulo, color, colorF1, colorF2, barras, semanasCosecha,
}: Props) {
  if (barras.length === 0) {
    return (
  ...
  const maxPlantas = Math.max(...barras.map(b => b.plantas_f1 + b.plantas_f2), 1);
  ...
  function xBar(sem: number) {
    return PAD_L + (sem - 1) * gap + gap / 2;
  }
  function yH(plantas: number) {
    return (plantas / maxPlantas) * chartH;
  }

  // Líneas de referencia Y
  const yRef = [0, Math.round(maxPlantas * 0.5), maxPlantas];

  // Semana de cosecha en X
      ...
      <svg viewBox={`0 0 ${W} ${H}`} style={{ width: '100%', height: 'auto', overflow: 'visible' }}>
        ...
        {yRef.map((v, i) => (
          <g key={i}>
            <line
              x1={PAD_L} x2={W - PAD_R}
              y1={PAD_T + chartH - yH(v)}
              y2={PAD_T + chartH - yH(v)}
              stroke="#f0f0f0" strokeWidth={1}
            />
            <text
              x={PAD_L - 4}
              y={PAD_T + chartH - yH(v) + 4}
        ...
        {semanas.map((sem) => {
          ...
          return (
            <g key={sem}>
              {/* F2 (abajo) */}
              {f2 > 0 && (
                <rect x={x} y={baseY - hF2} width={barW} height={hF2} fill={colorF2} rx={2} />
              )}
              {/* F1 (encima de F2) */}
              {f1 > 0 && (
                <rect x={x} y={baseY - hF2 - hF1} width={barW} height={hF1} fill={colorF1} rx={2} />
              )}
              {/* Total encima si hay datos */}
              {(f1 + f2) > 0 && (
                <text x={xBar(sem)} y={baseY - hF2 - hF1 - 3} textAnchor="middle" fontSize={9} fill="#4b5563" fontWeight={500}>
                  {f1 + f2 > 999 ? Math.round((f1 + f2) / 100) / 10 + 'k' : f1 + f2}
                </text>
              )}
            </g>
          );
        ...
        <line
          x1={xCosecha} x2={xCosecha}
          y1={PAD_T - 4} y2={PAD_T + chartH}
          stroke="#ef4444" strokeWidth={1.5} strokeDasharray="4 3"
        />
        <text x={xCosecha} y={PAD_T - 6} textAnchor="middle" fontSize={9} fill="#ef4444" fontWeight={600}>
          cosecha
        </text>
