...
export default async function AdminNavesPage() {
  ...
  let rows: any[] = [];
  try { rows = await readSheet('Naves'); } catch {}
  return (
    <>
      <Header user={user} current="admin" />
      <div className="container">
        <Link href="/admin" style={ fontSize: '13px', display: 'inline-block', marginBottom: '14px' }>← Admin</Link>
        <h1 className="page-title">Naves y mesadas</h1>
        <p className="page-subtitle">Capacidades físicas de cada mesada · {rows.length} registros</p>
        <div className="alert-box info">Gestión de naves y mesadas disponible. Edición directa en Google Sheets recomendada para cambios de estructura.</div>
        <div className="card">
