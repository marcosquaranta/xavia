        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ display: 'flex', height: '22px', alignItems: 'flex-end', borderBottom: '1px solid #e5e7eb' }}>
            {fechas.map((f, i) => {
              ...
              const esInicioSemana = i === 0 || d.getDay() === 1;
              const esPrimero = d.getDate() === 1;
              return (
                <div key={i} style={{
                  flex: 1, fontSize: '8px', color: esPrimero ? '#374151' : '#9ca3af',
                  fontWeight: esPrimero ? 700 : 400,
                  textAlign: 'center', overflow: 'hidden',
                  borderLeft: (esInicioSemana || esPrimero) ? '1px solid #e5e7eb' : 'none',
                  paddingBottom: '2px', lineHeight: 1,
                }}>
                  {(esInicioSemana || esPrimero) ? `${d.getDate()}/${d.getMonth() + 1}` : ''}
                </div>
              );
            })}
          </div>
          ...
          {filas.map((m, ri) => (
            <div key={m.nombre} style={{
              ...
              {m.dias.map((dia, j) => {
                ...
                const bg = dia.cultivo
                  ? COLOR[dia.cultivo]
                  : (esDom || esSab) ? '#f3f4f6' : 'white';
                return (
                  <div
                    key={j}
                    style={{
