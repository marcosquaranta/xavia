...
import { cosechadoEsteMes, plantasPorCultivo, variacionVsMesAnterior } from '@/lib/estadisticas';
...
export default async function PanelPage({ searchParams }: { searchParams: { filtro?: string; nave?: string } }) {
  ...
  try { [lotes, movimientos, ubicaciones, variedades] = await Promise.all([readSheet<Lote>('Lotes'), readSheet<Movimiento>('Movimientos'), readSheet<Ubicacion>('Ubicaciones'), readSheet<Variedad>('Variedades')]); } catch {}
  ...
  try { const m = cosechadoEsteMes(lotes); cosechadoMes = m.actual; cosechadoMesPasado = m.pasado; navesOcup = ocupacionPorNave(ubicaciones, lotes); resumen = plantasPorCultivo(lotes); varL = variacionVsMesAnterior(lotes, 'lechuga'); varR = variacionVsMesAnterior(lotes, 'rucula'); } catch {}
  const difPct = cosechadoMesPasado > 0 ? Math.round(((cosechadoMes - cosechadoMesPasado) / cosechadoMesPasado) * 100) : 0;
  const ocGlobal = navesOcup.length > 0 ? navesOcup.reduce((a: number, n: any) => a + n.plantas_vivas, 0) / Math.max(1, navesOcup.reduce((a: number, n: any) => a + n.capacidad_total, 0)) : 0;
    ...
    <>
      ...
      <div className="container">
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            <div style={{ background: 'white', border: '1px solid #e5e7eb', borderRadius: '10px', padding: '16px', flex: 1 }}>
              ...
              <div style={{ fontSize: '11px', color: '#6b7280', lineHeight: 1.7 }}>
                {navesOcup.map((n: any) => <div key={n.nave}>Nave {n.nave}: <strong style={{ color: '#1f2937' }}>{n.ocupacion_pct}%</strong><span style={{ color: '#9ca3af' }}> · {n.densidad_actual} pl/m²</span></div>)}
              </div>
            </div>
            <div style={{ background: 'white', border: '1px solid #e5e7eb', borderRadius: '10px', padding: '16px', flex: 1 }}>
              ...
              {cosechadoMesPasado > 0 && <p style={{ margin: 0, fontSize: '12px', color: difPct >= 0 ? '#059669' : '#dc2626', fontWeight: 500 }}>{difPct >= 0 ? '↑' : '↓'} {Math.abs(difPct)}% vs mes anterior</p>}
            </div>
          </div>
        </div>
        <div className="card">
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
          <h2 style={{ margin: 0, fontSize: '16px', fontWeight: 600 }}>Cultivos activos {filtro !== 'todos' || nave !== 'todas' ? '— ' + lotesFiltrados.length + ' de ' + conteos.todos : '(' + conteos.todos + ')'}</h2>
        </div>
...
function CultivoCard({ titulo, color, colorBg, datos, variacion, tieneFase1 }: { titulo: string; color: string; colorBg: string; datos: { plantinera: number; fase_1: number; fase_2: number; total: number }; variacion: number | null; tieneFase1: boolean }) {
  ...
  return (
    <div style={{ background: 'white', border: '1px solid #e5e7eb', borderTop: '3px solid ' + color, borderRadius: '10px', padding: '16px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
        ...
        <span style={{ fontSize: '22px', fontWeight: 700, color: '#111827' }}>{datos.total.toLocaleString('es-AR')}<span style={{ fontSize: '11px', fontWeight: 400, color: '#6b7280', marginLeft: '4px' }}>plantas</span></span>
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: tieneFase1 ? '1fr 1fr 1fr' : '1fr 1fr', gap: '6px', marginBottom: '10px' }}>
        {[['Plantinera', datos.plantinera], ...(tieneFase1 ? [['Fase 1', datos.fase_1]] : []), ['Fase 2', datos.fase_2]].map(([label, val]: any) => (
      ...
      <div style={{ paddingTop: '10px', borderTop: '1px solid #f3f4f6', fontSize: '12px', color: varColor, fontWeight: 500 }}>
        {variacion === null ? <span style={{ color: '#9ca3af', fontWeight: 400 }}>Sin datos del mes anterior</span> : <>{variacion >= 0 ? '↑' : '↓'} {Math.abs(variacion)}% vs cosechas mes anterior</>}
      </div>
    </div>
  );
}