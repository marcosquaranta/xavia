...
export default async function PanelPage({ searchParams }: {
  ...
  for (const l of lotes.filter(l => l.estado==='activo')) {
    ...
    // Buscar ciclo real
    const cicloEst = ciclosRealesMap.get(l.variedad) || ciclosRealesMap.get(varNorm) || (esR ? 35 : 80);
    if (diasSiembra > cicloEst * 1.1 && (l.fase_actual === 'fase_1' || l.fase_actual === 'fase_2')) {
      alertas.push({ tipo:'error', msg:`Lote ${l.id_lote} lleva ${diasSiembra}d (ciclo est. ${cicloEst}d) — pendiente cosechar`, lote: l.id_lote });
    }
  }

  // Sub-ocupación de mesadas (< 40%)
  for (const nave of tubosMesadas) {
    for (const m of nave.mesadas || []) {
      if (m.tubos_totales > 10 && m.ocupacion_pct < 40 && m.ocupacion_pct > 0) {
        alertas.push({ tipo:'warn', msg:`${m.nombre.replace(/^Nave \d+ - /,'')} al ${m.ocupacion_pct}% — sub-ocupada` });
      }
    }
  }
  ...
  for (const nave of tubosMesadas) {
    for (const m of nave.mesadas || []) {
      if (m.tubos_totales > 10 && m.tubos_ocupados === 0) {
        alertas.push({ tipo:'info', msg:`${m.nombre.replace(/^Nave \d+ - /,'')} vacía (0 tubos ocupados)` });
      }
    }
  }
